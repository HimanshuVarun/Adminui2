...
try {
	...
	System.out.println("exl first campaign in test class "+excel_user);
...
try {
  ...
  if(agent_coun>=AgentForm.agent_count)
     {
       logger = extent.startTest("agent");
	   //Assert.assertTrue(true);
   	   logger.log(LogStatus.PASS, "agent test case is pass ");
   	   logger.log(LogStatus.PASS, "total agent in database::"+agent_coun);
  ...
  else
     {
  	   logger = extent.startTest("agent");
	   //Assert.assertTrue(true);
	   logger.log(LogStatus.FAIL, "agent test case is fail because campaign is not available in database");
	   logger.log(LogStatus.FAIL, "total agent in database::"+agent_coun);
       logger.log(LogStatus.FAIL, "total agent in excel::"+AgentForm.agent_count);
   	   System.out.println("agent test case is fail because campaign not available in data base(some column is missing in table campaign)");
     }
 ...
 try {
      ...
      if(list_count>=ListsForm.list_co)
       {
...
loggerr.info("close uploadList test");
 ...
 //Assert.assertTrue(true);
 logger.log(LogStatus.PASS, "apply configuration test case is pass");
...
if(source_count>=Source_Lead.Source_count_excel)
 ...
 loggerr.info("close List_Scrubbing test");
  ...
  public void Dnc() throws Exception
  {
 	...
 	System.out.println("Dnc test case is start");
 logger = extent.startTest("Dnc");
 //Assert.assertTrue(true);
 logger.log(LogStatus.PASS, "Dnc test case is pass as it successfully add dnc_lists");
